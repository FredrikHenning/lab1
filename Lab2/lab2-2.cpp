...
//-------------------------------------------------------------------------//
// Store the rotation angles and number of lights here
float g_rotation[2];
int nr_lights;
//-------------------------------------------------------------------------//
...
static void key_callback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	...
	if ((key == GLFW_KEY_R) && action == GLFW_PRESS)
	{
		...
		std::string fragment_shader_str = readFile("../lab2-2_fs.glsl");
...
double lambertian_brdf(glm::vec3 inDirection, glm::vec3 outDirection, glm::vec3 normal) {
...
int main(int argc, char** argv)
{
	...
	int w_height = 600;
	int w_width = 800;
	nr_lights = 2; //initial value for the number of lights
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		/*glm::vec3 lightPosition(1.0f, 0.0f, 1.0f);	//Position
